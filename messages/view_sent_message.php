...
if (isset($_GET['id'])) {
    ...
    // Fetch message
    $stmt = $conn->prepare("SELECT m.subject, m.message, m.created_at, u.username as receiver FROM messages m JOIN users u ON m.receiver_id = u.id WHERE m.id = ? AND m.sender_id = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        header("Location: sent.php?error=Message not found");
        exit();
    }
    $stmt->close();
} else {
...
<div class="container">
    ...
    <a href="sent.php" class="btn btn-secondary mt-3">Back to Sent</a>
</div>
